package com.flowcloud.system.support;

public final class PermissionCodes {

    public static final String DASHBOARD = "dashboard";
    public static final String APPROVAL_SUBMIT = "approval:submit";
    public static final String APPROVAL_MY = "approval:my";
    public static final String APPROVAL_PENDING = "approval:pending";
    public static final String APPROVAL_ALL = "approval:all";
    public static final String TEMPLATE = "template";
    public static final String MESSAGES = "messages";
    public static final String SYSTEM_USER = "system:user";
    public static final String SYSTEM_DEPT = "system:dept";
    public static final String SYSTEM_TENANT = "system:tenant";
    public static final String SYSTEM_ROLE = "system:role";

    public static final String SYSTEM_USER_VIEW = "system:user:view";
    public static final String SYSTEM_USER_EDIT = "system:user:edit";
    public static final String SYSTEM_DEPT_VIEW = "system:dept:view";
    public static final String SYSTEM_DEPT_EDIT = "system:dept:edit";
    public static final String SYSTEM_TENANT_VIEW = "system:tenant:view";
    public static final String SYSTEM_TENANT_EDIT = "system:tenant:edit";
    public static final String SYSTEM_ROLE_VIEW = "system:role:view";
    public static final String SYSTEM_ROLE_EDIT = "system:role:edit";
    public static final String SYSTEM_DICT = "system:dict";
    public static final String SYSTEM_DICT_EDIT = "system:dict:edit";
    public static final String REPORT = "report";
    public static final String SYSTEM_MESSAGE_TEMPLATE = "system:message-template";
    public static final String SYSTEM_MESSAGE_TEMPLATE_EDIT = "system:message-template:edit";
    public static final String APPROVAL_TEMPLATE_MANAGE = "approval:template:manage";
    public static final String APPROVAL_INSTANCE_VIEW_ALL = "approval:instance:viewAll";
    public static final String APPROVAL_TASK_HANDLE = "approval:task:handle";

    private PermissionCodes() {
    }
}