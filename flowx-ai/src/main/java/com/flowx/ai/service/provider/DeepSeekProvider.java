...
@Slf4j
@Component
@ConditionalOnProperty(prefix = "flowx.ai.deepseek", name = "api-key")
public class DeepSeekProvider implements AIProvider {
    ...
    public DeepSeekProvider(AiProviderConfig aiProviderConfig, ObjectMapper objectMapper) {
        this.config = aiProviderConfig.getDeepseek();
        this.objectMapper = objectMapper;
        this.webClient = WebClient.builder()
                .baseUrl(config.getBaseUrl() != null ? config.getBaseUrl() : "https://api.deepseek.com/v1")
    ...
    public String chat(String prompt, List<Map<String, String>> context) {
        try {
            ...
            throw new BizException(ResultCodeEnum.AI_SERVICE_ERROR.getCode(), "DeepSeek返回结果为空");
        } catch (BizException e) {
            throw e;
        } catch (Exception e) {
            log.error("DeepSeek API call failed", e);
            throw new BizException(ResultCodeEnum.AI_SERVICE_ERROR.getCode(), "DeepSeek服务调用失败: " + e.getMessage());
        }
    }
