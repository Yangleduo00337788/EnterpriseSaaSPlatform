...
/**
 * Qwen provider implementation (Aliyun DashScope OpenAI-compatible API)
 *
 * @author FlowX
 * @since 1.0.0
 */
@Slf4j
@Component
@ConditionalOnProperty(prefix = "flowx.ai.qwen", name = "api-key")
public class QwenProvider implements AIProvider {
    ...
    public QwenProvider(AiProviderConfig aiProviderConfig, ObjectMapper objectMapper) {
        this.config = aiProviderConfig.getQwen();
        this.objectMapper = objectMapper;
        this.webClient = WebClient.builder()
                .baseUrl(config.getBaseUrl() != null ? config.getBaseUrl() : "https://dashscope.aliyuncs.com/compatible-mode/v1")
    ...
    public String chat(String prompt, List<Map<String, String>> context) {
        try {
            ...
            throw new BizException(ResultCodeEnum.AI_SERVICE_ERROR.getCode(), "Qwen返回结果为空");
        } catch (BizException e) {
            throw e;
        } catch (Exception e) {
            log.error("Qwen API call failed", e);
            throw new BizException(ResultCodeEnum.AI_SERVICE_ERROR.getCode(), "Qwen服务调用失败: " + e.getMessage());
        }
    }
